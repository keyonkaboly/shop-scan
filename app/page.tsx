...
export default function Home() {
  ...
  return <main className="site-shell">
    ...
    <section className="results-section" id="results"><div className="results-topline"><div><p className="eyebrow"><span className="eyebrow-line" /> MARKET RESULTS / {condition.toUpperCase()}</p><h2>{isLoading ? "Scanning the market." : "Cheapest today."}</h2></div><div className="price-lockup"><span>from</span><strong>{lowestPrice ? `${liveListings[0].currency} ${lowestPrice}` : "—"}</strong></div></div>{error && <p className="scan-error">{error}</p>}<div className="listings-list">{isLoading && <div className="scan-status">Checking eBay and Grailed for IT {sizeIT} / US {sizeUS}...</div>}{!isLoading && visibleRows.map(renderRow)}{!isLoading && resultRows.length === 0 && <div className="scan-status">No verified live listings found for this selection.</div>}</div>{!isLoading && pageCount > 1 && <div className="pagination" aria-label="Marketplace result pages"><button type="button" onClick={() => setResultPage((page) => Math.max(0, page - 1))} disabled={resultPage === 0}>Previous</button><span>{resultPage * pageSize + 1}–{Math.min((resultPage + 1) * pageSize, resultRows.length)} of {resultRows.length}</span><button type="button" onClick={() => setResultPage((page) => Math.min(pageCount - 1, page + 1))} disabled={resultPage === pageCount - 1}>Next</button></div>}<div className="results-footer"><span>{isLoading ? "Scan in progress" : `${liveListings.length} verified listings · ${manualSources.filter(([name]) => unavailable.has(name)).length} manual searches`}</span><span>{scan.scannedAt ? `Checked ${new Date(scan.scannedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : "Waiting for scan"} <i /></span></div></section>
