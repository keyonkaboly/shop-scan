...
export default function Home() {
  ...
  const filteredListings = liveListings.filter((listing) => (minPrice == null || priceOf(listing) >= minPrice) && (maxPrice == null || priceOf(listing) <= maxPrice));
  ...
  const resultRows: ResultRow[] = [...filteredListings.map((listing) => ({ type: "live" as const, listing })), ...ebayFallback, ...manualSources.filter(([name]) => unavailable.has(name)).map(([name, url]) => ({ type: "manual" as const, name, url, status: directLinkSources.has(name) ? "Direct link added" : "No direct link yet" }))];
  const pageSize = 10;
