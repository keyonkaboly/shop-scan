...
export default function Home() {
  ...
  const renderRow = (row: ResultRow, index: number) => {
    ...
    return <a className="listing-row" href={listing.url} target="_blank" rel="noopener noreferrer" key={`${listing.marketplace}-${listing.url}`}><span className="listing-rank">{String(resultPage * pageSize + index + 1).padStart(2, "0")}</span><span className="listing-image">{thumbnailUrl ? <img src={thumbnailUrl} alt={`${listing.marketplace} listing: ${listing.title}`} loading="lazy" /> : <span aria-hidden="true" />}</span><span className="listing-info"><strong>{listing.marketplace}</strong><span>{listing.title}</span></span><span className="listing-badge">{listing.sourceType === "api" ? "Live API" : "Live scan"}</span><span className="listing-price">{originalPrice} <small>{cadPrice}</small></span><span className="listing-arrow">↗</span></a>;
  };
  ...
  return <main className="site-shell">
    ...
    <section className="results-section" id="results"><div className="results-topline"><div><p className="eyebrow"><span className="eyebrow-line" /> MARKET RESULTS / {condition.toUpperCase()}</p><h2>{isLoading ? "Scanning the market." : "Cheapest today."}</h2></div><div className="price-lockup"><span>from</span><strong>{lowestPrice ? `${liveListings[0].currency} ${lowestPrice}` : "—"}</strong></div></div>{error && <p className="scan-error">{error}</p>}{!isLoading && !unavailable.has("eBay") && liveListings.every((listing) => listing.marketplace !== "eBay") && <p className="source-note">eBay live API checked · no verified listing matched this size and condition.</p>}<div className="listings-list">{isLoading && <div className="scan-status">Checking eBay and Grailed for IT {sizeIT} / US {sizeUS}...</div>}{!isLoading && visibleRows.map(renderRow)}{!isLoading && resultRows.length === 0 && <div className="scan-status">No verified live listings found for this selection.</div>}</div>{!isLoading && pageCount > 1 && <div className="pagination" aria-label="Marketplace result pages"><button type="button" onClick={() => setResultPage((page) => Math.max(0, page - 1))} disabled={resultPage === 0}>Previous</button><span>{resultPage * pageSize + 1}–{Math.min((resultPage + 1) * pageSize, resultRows.length)} of {resultRows.length}</span><button type="button" onClick={() => setResultPage((page) => Math.min(pageCount - 1, page + 1))} disabled={resultPage === pageCount - 1}>Next</button></div>}<div className="results-footer"><span>{isLoading ? "Scan in progress" : `${liveListings.length} verified listings · ${manualSources.filter(([name]) => unavailable.has(name)).length} manual searches`}</span><span>{scan.scannedAt ? `Checked ${new Date(scan.scannedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : "Waiting for scan"} <i /></span></div></section>
    ...
    <footer><span className="wordmark"><span className="wordmark-mark" aria-hidden="true"><i /></span>GAT SCAN</span><span>MAISON MARGIELA REPLICA GAT / 2026</span><span>Built for the careful customer.</span></footer>
  </main>;
}
